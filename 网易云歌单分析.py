...
class MyFrame1 (threading.Thread,wx.Frame):
	...
	def __init__( self, threadID, name ,counter):
		wx.Frame.__init__ ( self, None, id = wx.ID_ANY, title = u"网易云音乐歌单分析", pos = wx.DefaultPosition, size = wx.Size( 450,409 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
		...
		self.down_button = wx.Button( self, wx.ID_ANY, u"分析", wx.DefaultPosition, wx.DefaultSize, 0 )
	...
	def get(self,values):
		...
		for x in values:
			...
			if not os.path.exists("d:/music/" + x['name'] + '.mp3'):
				self.output_text.AppendText(x['name']+"\n")
				print("\n")
		...
		self.output_text.AppendText('Analysis complete ' + str(downNum) + ' files !\n')
		pass
